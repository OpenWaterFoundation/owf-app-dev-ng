...
import { Parameters } from './gapminder-util/get-parameters.js';
...
export class GapminderViz {
  ...
  public gapminder() {
     ...
     */
    var properties = new Properties(this.configPath);
     ...
     * Also parses through csv data read in and convert to JSON for Gapminder
     */
    ...
    var currYear = dimensions.dateMin;   //made global for back and forward functions
    var topYear = dimensions.dateMin;
     ...
     * TimeStep: 1Year. Returns '1'
     */
    (window as any).precisionInt = parsePrecisionInt(properties.TimeStep);
    /**
    ...
    dimensions.maxPopulatedDate.setHours(23, 59, 59);
    ...
    var div = d3.select("#Gapminder").append("div")
      .attr("class", "tooltip")
      .style("opacity", 0);
    // $(window).click(function(e){ // .click deprecated
    ...
    d3.select("#maintitle")
      ...
      .append("text")
      .text(formatDate((window as any).currYear));
    ...
    var timeScale = d3.scaleTime()
      .domain([dimensions.dateMin, dimensions.dateMax])
      .range([0, (width - 75)]);
    ...
    if (dateArray[dateArray.length - 1].getTime() !== dimensions.dateMax.getTime()) {
      dateArray.push(dimensions.dateMax);
    }
    ...
    slider.append("line")
      ...
      .attr("cursor", "crosshair")	//  CSS Styling for dateslider
      ...
      .on("mousemove", function (event) {	// event passed as first argument to all listeners 
        ...
        if (date <= dimensions.maxPopulatedDate) {
    ...
    var handle = slider.insert("circle", ".track-overlay")
      .attr("class", "handle")
      .attr("r", 5)
      .attr("fill", "#fff")	// CSS Styling for date slider: default handle color is black, this makes it white
    ...
    var yText = d3.select(".box")
      .append("text")
      .text(d3.format(",.0f")(Math.round(dimensions.yMax))) //hard-coded, needs to change
      .attr("fill-opacity", 0);
    //create svg element of largest x-axis label
    var xText = d3.select(".box")
      .append("g")
      .append("text")
      .text(d3.format(",.0f")(Math.round(dimensions.xMax))) //hard-coded, needs to change
      .attr("fill-opacity", 0);
    ...
    var radiusScale = d3.scaleSqrt().domain([0, (dimensions.radiusMax * 2)]).range([3, 47]);
    //assign xScale according to if Config file specifies Log or Linear
    if (properties.XAxisScale.toUpperCase() == "LOG") {
      var min = properties.XMin && properties.XMin != "" ? properties.XMin : dimensions.xMin;
      var max = properties.XMax && properties.XMax != "" ? properties.XMax : dimensions.xMax;
    ...
    if (properties.YAxisScale.toUpperCase() == "LOG") {
      var min = properties.YMin && properties.YMin != "" ? properties.YMin : dimensions.yMin;
      var max = properties.YMax && properties.YMax != "" ? properties.YMax : dimensions.yMax;
    ...
    if (annotations_data != null && !$.isEmptyObject(annotations_data.SpecificAnnotations)) {
      ...
      if (symbolAnnotations) {
        ...
        if (circleAnnotations) {
          var annotationCircle = svg.append("g")
            .selectAll(".point")
        ...
        if (triangleAnnotations) {
          var annotationTriangle = svg.append("g")
            .selectAll(".point")
    ...
    var nested = nest(interpolatePath(dimensions.dateMin), pathData);
    ...
    function add_dots() {
      ...
      dot = dot_g.selectAll(".dot")
        .data(interpolateData(dimensions.dateMin))
        .enter().append("circle")
        ...
        .on("contextmenu", function (event: any, d: any) {	// event passed as first argument to all listeners 
          if (properties.TimeseriesEnabled) {
            ...
            div.html("<p style='margin:0px;'><a style='color:black; font-weight:bold;' href='./highchart.html?csv=" +
              expand_parameter_value(properties.FilePropertyName, { "Year": properties.DefaultDatasetChoice }) +
              "&name=" + d.name + "&nameVar=" + variables.Label + "&xVar=" + variables.XAxis + "&yVar=" + variables.YAxis +
              "&size=" + variables.Sizing + "&datevariable=" + variables.Label + "', target='_blank'> Timeseries</a></p>")
        ...
        .attr("display", "true")
        .call(position) //position callback
    ...
    if (Properties.DefaultSpeed) {
      ...
      (<HTMLInputElement>document.getElementById("speedSlider")).value = Properties.DefaultSpeed;
    }
     ...
     */
    function mousedown(event: any, d: any) {	// event passed as first argument to all listeners 
    ...
    /**
     *Callback Function: Called when user hovers over dot with the mouse </p>
    ...
    function mouseover(event: any, d: any) {	// Updated: event passed as first argument to all listeners 
      ...
      if (d3.select(this).attr("display") == "true") {
        ...
        d3.select(this).raise(); // .moveToFront();
      ...
      if (properties.AdditionalData) {
        ...
        if (additionalParameters) {
          ...
          for (var index = 0; index < showColumns.length; index++) {
            tableContents += "<p><strong>" + showColumns[index] + "</strong>: " + additionalParameters[showColumns[index]] + "</p>";
          }
        }
      }
    ...
    */
    function mouseout(event: any, d: any) {	// Updated: event passed as first argument to all listeners 
    ...
    function position(dot: any) {
      dot.attr("cx", function (d: any) {
        ...
        .attr("r", (d: any) => minRadius(radiusScale(radius(d))));
    }
    ...
    function draggedYear(date: any) {
      ...
      if (date >= dimensions.dateMin && date < dimensions.maxPopulatedDate) {
        displayYear(date);
      ...
      else if (date > dimensions.maxPopulatedDate) {
        displayYear(dimensions.maxPopulatedDate);
      ...
      else if (date < dimensions.dateMin) {
        displayYear(dimensions.dateMin);
     ...
     *Called separately to create a synchronous ordering of things,
     * this way the global variables are properly configured before starting the animation again.
     */
    var replay = function () {
      pathData = [];
      updatePath(nest(interpolatePath(dimensions.dateMin), pathData));
      topYear = dimensions.dateMin;
      currYear = dimensions.dateMin;
      playAnimation();
    }
    ...
    function playAnimation() {
      ...
      var transitionFunction = svg.transition()
        .duration(getTimeInterpolate(currYear, dimensions.maxPopulatedDate)) //call getTimeInterpolate to calculate amount of time between years transition
        .ease(d3.easeLinear)
    ...
    var setSpeed = function (value: any) {
     ...
     */
    function tweenYear() {
      var interpolateDate = d3.interpolateDate(currYear, dimensions.maxPopulatedDate);
      return function (t) {
        if (t == 1) {
          transition = true;
        }
    ...
    var displayYear = function (date: any) {
     ...
     */
    function interpolatePath(year: any) {
     ...
     *
     *@param newData - an array of updated data
     */
    ...
    function specificPathData(data: any) {
      var returnThis = [];
      // for (let i = 0; i < properties.tracerNames.length; i++) {
      //   for (let j = 0; j < data.length; j++) {
      //     if (properties.tracerNames[i] == data[j][name]) {
    ...
    /**
     *Separates annotations file into different shape types </p>
    ...
    function retrieveAnnotations(shape: any) {
      ...
      var group = d3.group(annotations_data.SpecificAnnotations, (d: any) => d.ShapeType)
    ...
    function getIndividualDots(json: any) {
      ...
      var group = d3.group(json.data, (d) => d[variables.Label]);
     ...
     *@param year
     */
    function getAnnotations(year: any) {
     ...
     */
    function getTimeInterpolate(startDate: any, endDate: any) {
     ...
     */
    function timeDiff(date1: any, date2: any) {
      return Math.abs(date2.getTime() - date1.getTime());
    }

    // function roundDate(date){
    function roundDate(date: any) {
    ...
    function updateGapminder(date) {
      ...
      //update timeSlider
      //create a timescale for year slider
      timeScale.domain([dimensions.dateMin, dimensions.dateMax]);
      ...
      nested = nest(interpolatePath(dimensions.dateMin), pathData);
      add_path(nested); // path is missing???

      pauseButton();
      displayYear(dimensions.dateMin);
      /*pathData = [];
      updatePath(interpolatePath(dimensions.dateMin), pathData);*/
   ...
   */
    ...
    function backButton() {
      ...
      (<HTMLInputElement>document.getElementById("play")).disabled = false;
      (<HTMLInputElement>document.getElementById("forward")).disabled = false;
      if (currYear >= dimensions.dateMin) {
        stopAnimation();
        setTimeout(function () {
    ...
    /**
       ...
       */
    function decrementDate(date: any) {
    ...
    function forwardButton() {
      (<HTMLInputElement>document.getElementById("back")).disabled = false;
      if (currYear <= dimensions.maxPopulatedDate) {
     ...
     *Ensures smallest dots are above larger dots
     *
     *@param a - the svg dot to check against
     *@param b - the svg dot to check against
     */
    ...
    *
    *@param value - number to check minimum on
    */
    function minRadius(value: any) {
    ...
    */
     ...
     */
    function legendButton(d: any, selectMultiple: any) {
     ...
     */
    function x(d: any) { return d.xVar; }
    /**
     *Accessor function for y-variable
     */
    function y(d: any) { return d.yVar; }
    /**
     *Accessor function for size of dot
     */
    function radius(d: any) { return d.size; }
    /**
     ...
     */
    function checkForSymbol(string: any) {
     ...
     *ex: 'Denver Water' -> '.Denver.Water'
     */
    function dot_id_selector(inputString: any) {
      var string = inputString.split(" ");
      var returnThis = "#D";
      for (var i = 0; i < string.length - 1; i++) {
        if (checkForSymbol(string[i]) != "") {
     ...
     *ex: 'Denver Water' -> '.Denver.Water'
     */
    function dot_class_selector(inputString: any) {
      var string = inputString.split(" ");
      var returnThis = ".D";
      for (var i = 0; i < string.length - 1; i++) {
        if (checkForSymbol(string[i]) != "") {
     ...
     *ex: 'Denver Water' -> '.Denver.Water'
     */
    function path_id_selector(inputString: any) {
      var string = inputString.split(" ");
      var returnThis = "#T";
      for (var i = 0; i < string.length - 1; i++) {
        if (checkForSymbol(string[i]) != "") {
    ...
    function expand_parameter_value(parameter_value: any, properties: any) {
      ...
      while (search_pos < parameter_value.length) {
        var found_pos_start = parameter_value.indexOf(delim_start),
          ...
          prop_name = parameter_value.substring((found_pos_start + 2), ((found_pos_end - found_pos_start) - 2)),
        ...
        b = parameter_value.substring(0, found_pos_start) + prop_val + parameter_value.substring(found_pos_end + 1, parameter_value.length);
     ...
     */
    function incrementDate(date: any) {
      switch (precisionUnit) {
        case "Year":
          date.setFullYear(date.getFullYear() + precisionInt);
          break;
    ...
    *@param array - an array containing data
    */
    function nest(data: any, array: any) {
